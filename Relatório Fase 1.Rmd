...
```{r rf}
crop.df <- read_csv("crop_df.csv")
crop.df$material <- substr(crop.df$material, 1, 1) # try to balance
barplot(table(crop.df$material)) # still not balanced, but let's give it a shot
crop.df[, 1:14] <- lapply(crop.df[, 1:14], as.factor)
...
set.seed(300)
dim(crop.df)
m.rf <- train(productivity ~ ., data = crop.df, method = "rf",
              metric = "RMSE", trControl = ctrl, tuneGrid = grid)
m.rf
yhat.rf <- predict(m.rf, crop.df)
plot(crop.df$productivity, yhat.rf)
abline(lm(yhat.rf ~ crop.df$productivity), col = 'darkred', lwd = 2)
cor(crop.df$productivity, yhat.rf)^2
