...
# Somente soja
crop.df <- read_csv("crop_df.csv", col_types = cols(epoca = col_character()))
crop.df$material <- substr(crop.df$material, 1, 1)
barplot(table(crop.df$material))
# write_csv(crop.df, 'crop.kn.csv')

crop.df$safra <- NULL #  info temporal
crop.df <- crop.df %>% mutate_if(is.character, factor)
library(Boruta)
set.seed(1)
feat.sel <- Boruta(productivity ~ ., data = crop.df, doTrace = 1)
table(feat.sel$finalDecision)
fnames <- getSelectedAttributes(feat.sel)
fnames
filt.crop <- crop.df[, c(fnames, "productivity")]
